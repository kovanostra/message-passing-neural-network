...
__global__ void compose_messages_kernel(
    ...
    for (int node_id = index; node_id < number_of_nodes; node_id += stride) {
      for (int end_node_index = 0; end_node_index<(int)all_neighbors[node_id].sizes()[0]; end_node_index++){
        auto end_node_id = std::round(all_neighbors[node_id][end_node_index]);
        if (end_node_id >= 0) {
          for (int neighbor_index = 0; neighbor_index<(int)all_neighbors[node_id].sizes()[0]; neighbor_index++) {
